...
export function drawStaticBackground() {
  ...
  function drawGrass(context, camera) {
    ...
    for (let i = 0; i < 30; i++) {
      context.drawImage(
        GlassBackImage,
        GlassCoordX + GrassImage.width * i + 600,
        buffer.height - GlassBackImage.height + 1
      );
      context.drawImage(
        GrassImage,
        GrassCoordX + GrassImage.width * i,
        buffer.height - GrassImage.height + 1
      );
      context.drawImage(
        GlassFrontImage,
        GlassCoordX + GrassImage.width * i + 100,
        buffer.height - GlassFrontImage.height + 1
      );
    }
  }
